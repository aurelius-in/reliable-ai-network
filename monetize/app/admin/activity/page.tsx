...
export const metadata = { title: "Activity Counter — Make it RAIN" };

type SearchParams = Promise<{ key?: string; range?: string }>;

export default async function AdminActivityCounterPage({
  searchParams,
}: {
    ...
    return (
      <Shell>
        <div className="rounded-2xl border border-night-600 bg-night-800 p-8 text-center">
          <h1 className="text-xl font-bold text-white">Activity Counter</h1>
          <p className="mt-2 text-sm text-slate-400">
            Add your admin key to the URL:
            <br />
            <code className="mt-2 inline-block text-rain-bright">
              /admin/activity?key=YOUR_SECRET
            </code>
          </p>
        </div>
      </Shell>
      ...
      <Shell>
        <div className="rounded-2xl border border-red-500/30 bg-night-800 p-8 text-center">
          <h1 className="text-xl font-bold text-white">
            Could not load Activity Counter
          </h1>
  ...
  return (
    <Shell>
      <CounterDashboard
        ...
        subtitle={`Accounts + site activity · ${stats.rangeLabel} · shareable view`}
        showRecentAccounts={false}
        bookmarkPath="/admin/activity"
      />
    </Shell>
  );
}
